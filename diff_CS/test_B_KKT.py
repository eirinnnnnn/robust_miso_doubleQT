...
from functions import update_B_loop_robust_maxmin, update_B_loop_robust_stableB,update_B_loop_robust, compute_rate_test, compute_rate_over, compute_g1_k_QT
...
def test_update_B_loop():
    ...
    h_hat_id=2
    # constants = GlobalConstants(snr_db=0, snrest_db=[-20, -20], Nt=32, Nr=2, K=2, Pt=1, Pin=0.75, H_HAT=H[h_hat_id], h_hat_id=h_hat_id)
    constants = GlobalConstants(snr_db=5, snrest_db=[-20, -20], Nt=32, Nr=2, K=2, Pt=2, Pin=0.75, H_HAT=H[h_hat_id], h_hat_id=h_hat_id)
    ...
    # print(np.linalg.norm(constants.H_HAT[0]), np.linalg.norm(constants.H_HAT[1]))
    ...
    # B_robust, lagrangian_B_trajectory, alpha_trajectory, beta_trajectory , t_trajectory, res1, res2= update_B_loop_robust_stableB(
        A_robust, B_robust, constants, max_outer_iter=20000, outer_tol=1e-5, max_inner_iter=2000,inner_tol=1e-4, robust=True
    )
    ...
    # B_nonrobust, non_robust_lag,_,non_beta_traj,_,_,non_res2_traj = update_B_loop_robust_stableB(A_nonrobust, B_nonrobust, constants, 
    # B_nonrobust, non_robust_lag,_,_,_,_,_,_,_,_ = modified_update_B_loop_robust(A_nonrobust, B_nonrobust, constants, 
                                        ...
                                        # max_outer_iter=20000, outer_tol=1e-4, 
