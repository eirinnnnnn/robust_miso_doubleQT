...
def test_update_B_loop():
    ...
    h_hat_id=2
    snr= -20
    constants = GlobalConstants(snr_db=5, snrest_db=[snr, snr], Nt=32, Nr=2, K=2, Pt=2, Pin=0.75, H_HAT=H[h_hat_id], h_hat_id=h_hat_id)
    ...
    # B_robust, lagrangian_B_trajectory, alpha_trajectory, beta_trajectory , t_trajectory, res1, res2= update_B_loop_robust_stableB(
        A_robust, B_robust, constants, max_outer_iter=20000, outer_tol=1e-6, max_inner_iter=2000,inner_tol=1e-4, robust=True
    )
    ...
    # Rate calculation
    r, r_m, r_v, r_o = compute_rate_test(A_robust, B_robust, constants, Delta_k, snr, samp=5000)
    n, n_m, n_v, n_o = compute_rate_test(A_nonrobust, B_nonrobust, constants, Delta_k, snr, samp=5000)
    w, w_m, w_v, w_o = compute_rate_test(A_nonrobust, B_wmmse, constants, Delta_k, snr, samp=5000)
    z, z_m, z_v, z_o = compute_rate_test(A_nonrobust, B_zf, constants, Delta_k, snr, samp=5000)
    # w = []
    # w_m=0
    # w_v=0
    # w_o=0
    ...
    # print(f"var: robust={r_v}, non={n_v}, wmmse={w_v}, zf={z_v}")
    ...
    slack_pwr = constants.PT-sum(torch.linalg.norm(B_robust.V[k]) ** 2 for k in range(constants.K))
